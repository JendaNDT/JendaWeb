...
V `data.js` má všech 15 skladeb `audioUrl: null`, takže přehrávač zatím nehraje. Postup je v `handoff/AGENT_CONTEXT.md` (sekce „Přidat reálné audio URL ze Suno").

## ✅ Hotovo
- **Nasazeno živě na Vercel** (`jenda-web.vercel.app`) přes GitHub auto-deploy — push do `main` = automatický deploy
- **Offline soběstačnost:** React/ReactDOM/Babel + fonty Syne/DM Sans hostované lokálně (`vendor/`), service worker precachuje celý boot (`jw-v17`). Web nepotřebuje žádné CDN.
...
- **Po změně cachovaného souboru bumpni `VERSION` v `sw.js`** (teď `jw-v23`).
...
- `sw.js` – service worker (`jw-v17`) · `manifest.webmanifest`
